...
public class KnowledgeController {
    ...
    String knowledgePage(@PathVariable("pro_id") String proId, Model model) {
        ...
        professionClassification.setEnergy(energy);
...
//        model.addAttribute("knowledgeContent", cur.getKnowledgeContent());
    ...
    CommonMessage modify(@PathVariable("kn_id") String knowId, @RequestBody Knowledge knowledge) {
        ...
        Knowledge knowledge1 = knowledgeRepository.
                findByKnowledgeNameAndProfessionIdAndKnowledgeIdNot(knowledge.getKnowledgeName(),
                        knowledge.getProfessionId(), knowId);
        if (knowledge1 != null){
            return CommonMessage.failure("该职业下已有此知识点名..");
        }
    ...
    CommonMessage add(@PathVariable("pro_id") String proId, @RequestBody Knowledge knowledge) {
        Knowledge knowledge1 = knowledgeRepository.findByKnowledgeNameAndProfessionId(knowledge.getKnowledgeName(), proId);
        if (knowledge1 != null){
            return CommonMessage.failure("该职业下已有此知识点名..");
        }
...
//        model.addAttribute("contents", gson.toJson(contents.stream().collect(Collectors.groupingBy(ContentBaseView::getId))));
        ...
        kn.setKnowledgeContent(new KnowledgeContent(Collections.emptyList(), Collections.emptyList()));

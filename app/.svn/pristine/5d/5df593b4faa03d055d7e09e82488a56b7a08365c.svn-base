...
public class IconContrller {
    ...
    @ResponseBody
    public String getIcons(HttpServletRequest request, int pageNum, String name) {
        Sort sort = new Sort(Sort.Direction.DESC, "createTime");
        Pageable pageable = new PageRequest(pageNum - 1, 10, sort);
        Page<IconManagement> list;
        if (StringUtils.isEmpty(name)) {
            list=iconRepository.findByState(pageable,true);
        } else {
            list = iconRepository.findByStateAndNameLike(pageable,true, "%" + name + "%");
        }
    ...
    @ResponseBody
    public String saveIcon(@ModelAttribute IconManagement iconManagement) {
        iconManagement.setState(true);
